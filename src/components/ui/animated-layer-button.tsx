...
export interface AnimatedLayerButtonProps
  ...
  /** When true, force the hover state (used to play the animation on mobile click before opening a modal). */
  forceActive?: boolean;
}
...
>(({ className, children, forceActive = false, ...props }, ref) => {
  return (
    <button
      ...
      className={cn(
        "group relative flex h-[52px] min-w-[200px] items-center justify-center overflow-hidden rounded-[30px] border-none px-7",
        "cursor-pointer bg-cobalt shadow-[8px_8px_0px_hsl(var(--foreground))] transition-all duration-300 ease-in-out",
      ...
      {...props}
    >
      {/* Spinning SVG layer that expands across the button */}
      <svg
        className={cn(
          "absolute h-auto transition-all duration-300 ease-in-out",
          "w-[60px] -left-[30px]",
          "group-hover:left-0 group-hover:w-full",
          "group-data-[active=true]:left-0 group-data-[active=true]:w-full",
          "animate-spin-slow",
        )}
      ...
      <span
        className={cn(
          "relative z-10 font-semibold text-primary-foreground transition-colors duration-300 text-[1.05em] tracking-wide",
          "group-hover:text-transparent",
          "group-data-[active=true]:text-transparent",
        )}
      >
        {children}
      </span>
